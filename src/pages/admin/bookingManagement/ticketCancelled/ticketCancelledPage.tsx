...
const [selectedRowKey, setSelectedRowKey] = useState<number | null>(null);
  ...
  return (
    <div style={{ padding: "20px" }}>
      <h2 style={{ textAlign: "center", marginBottom: "20px", color: "#ff4d4f" }}>Danh sách vé đã hủy</h2>
      <Table
  rowKey="tickerId"
  loading={loading}
  dataSource={data}
  ...
  onRow={(record) => ({
    onClick: () => {
      setViewingTicket(record); // Hiển thị modal
      setSelectedRowKey(record.tickerId); // Lưu hàng được chọn
    },
    style: {
      cursor: "pointer",
      backgroundColor: selectedRowKey === record.tickerId ? "#e6f7ff" : "transparent", // Bôi màu nếu được chọn
    },
  })}
/>
<Modal
